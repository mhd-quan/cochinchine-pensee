...
const script = (file) =>
  ts.transpileModule(
    fs
      .readFileSync(new URL(`../src/components/essay/${file}`, import.meta.url), 'utf8')
...
function navigation() {
  ...
  const elements = {
    '[data-reader-return]': button,
    '[data-article-end]': { getBoundingClientRect: () => ({ bottom: state.bottom }) },
    '#essay-title': {
      focus: () => {
        state.focused = true;
      },
  ...
  const window = Object.assign(new EventTarget(), {
    scrollY: 0,
    innerHeight: 800,
    scrollTo: (options) => state.calls.push(options),
  });
  let next = 0;
  const frames = new Map();
  vm.runInNewContext(script('ReaderNavigation.astro'), {
    document,
    window,
...
test('return control respects reduced motion, transfers focus, and cleans up on navigation', () => {
  ...
  assert.equal(h.state.calls[0].top, 0);
  assert.equal(h.state.calls[0].behavior, 'instant');
...
test('platform controls open only on activation and do not duplicate handlers after Astro navigation', () => {
  ...
  const buttons = [
    ...
    'https://twitter.com/intent/tweet?url=example',
  ].map((url) =>
    Object.assign(new EventTarget(), { dataset: { readerShare: url }, disabled: true }),
  );
  const document = Object.assign(new EventTarget(), {
    querySelectorAll: (s) => (s === '[data-reader-share]' ? buttons : []),
  });
  vm.runInNewContext(script('ShareLinks.astro'), {
    document,
