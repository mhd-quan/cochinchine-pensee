...
import {
  ...
  readArticle,
  textContent,
} from '../scripts/html-document.mjs';
...
const profiles = JSON.parse(
  fs.readFileSync(new URL('../editorial/essay-seo.json', import.meta.url)),
);
...
test('canonical URLs discard tracking, anchors and alternate index spelling', () => {
  assert.equal(
    canonicalUrl('/essays/example/index.html?utm_source=x#note'),
    'https://cochinchinepensees.studio/essays/example',
  );
...
test('rendered essays have truthful identity, consistent metadata, discovery and downloads', () => {
  ...
  for (const [slug, profile] of Object.entries(profiles)) {
    ...
    const meta = (key) =>
      metadata
        .filter((n) => attr(n, 'name') === key || attr(n, 'property') === key)
    ...
    const graphs = findAll(
      article.document,
      (n) => n.tagName === 'script' && attr(n, 'type') === 'application/ld+json',
    ).flatMap((n) => JSON.parse(textContent(n))['@graph'] ?? []);
    ...
    assert.ok(
      fs.existsSync(new URL(`${new URL(entity.author.url).pathname.slice(1)}/index.html`, dist)),
      slug,
    );
    for (const image of entity.image ?? [])
      assert.ok(fs.existsSync(new URL(new URL(image).pathname.slice(1), dist)), `${slug}: image`);
    ...
    const pdf = findAll(article.document, (n) => n.tagName === 'a').find(
      (n) => attr(n, 'href') === `/downloads/essays/${slug}.pdf`,
    );
    assert.ok(pdf, `${slug}: PDF link`);
    assert.equal(attr(pdf, 'data-astro-prefetch'), 'false');
    assert.ok(
      fs.existsSync(new URL(`downloads/essays/${slug}.pdf`, dist)),
      `${slug}: generated PDF`,
    );
...
test('PDF extraction preserves list labels, emphasis, images and footnote text', () => {
  const doc = parse(
    '<article><ol><li><p>First <em>word</em></p></li><li><p>Second</p></li></ol><p>Note<sup>1</sup></p><figure><img src="/images/a.webp"><figcaption>Caption</figcaption></figure><section><ol><li><p>Footnote <a href="#fnref-1">↩</a></p></li></ol></section></article>',
  );
