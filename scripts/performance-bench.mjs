...
if (!Number.isInteger(Number(runs)) || Number(runs) < 1) {
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (!item) {
      ...
      const compress = /text|json/.test(type);
      if (compress) data = brotliCompressSync(data);
      item = { data, type, compress };
      cache.set(file, item);
    }
    res.setHeader('Content-Type', item.type);
    res.setHeader('Cache-Control', 'no-store');
    if (item.compress) res.setHeader('Content-Encoding', 'br');
    res.end(item.data);
  } catch {
    res.statusCode = 404;
    res.end();
...
try {
  ...
  for (let i = 0; i < +runs; i++) {
    const context = await browser.newContext({
      viewport: desktop ? { width: 1440, height: 1000 } : { width: 390, height: 663 },
    ...
    if (failures.length || !result.lcp) {
      throw new Error(`Invalid benchmark run: ${failures.join('; ') || 'no LCP observed'}`);
    }
    result.cls = cumulativeLayoutShift(result.layoutShifts);
    result.metrics = (await cdp.send('Performance.getMetrics')).metrics.filter((m) =>
      /LayoutDuration|RecalcStyleDuration|ScriptDuration|TaskDuration|LayoutCount/.test(m.name),
    );
    if (traceEnabled && i === 0) {
      ...
      await fs.writeFile(`${output}/${label}-trace.json`, trace);
    ...
    JSON.stringify(
      {
        metadata: {
          ...
          viewport: desktop ? '1440x1000@1' : '390x663@3',
          cpuRate: 4,
          latencyMs: 170,
