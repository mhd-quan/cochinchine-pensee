...
import { articleBlocks, attr, readArticle } from './html-document.mjs';
...
async function prepareFonts() {
  ...
  for (const style of ['normal', 'italic', 'bold', 'bolditalic', 'cjk']) {
    const bytes = await fs.readFile(
      path.join(root, `scripts/assets/pdf-fonts/${style}.${style === 'cjk' ? 'otf' : 'ttf'}`),
    );
    const parsed = opentype.parse(
      bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength),
    );
    const points = new Set(
      Object.entries(parsed.tables.cmap.glyphIndexMap)
...
function segments(run, defaultStyle = 'normal') {
  ...
  for (const character of run.text.normalize('NFC').replace(/[\u200B-\u200D\uFEFF]/g, '')) {
    ...
    const font =
      fonts.find((f) => f.style === style && (f.points.has(point) || character === '\n')) ??
      fonts.find((f) => f.points.has(point));
    if (!font)
      throw new Error(
        `PDF font lacks ${JSON.stringify(character)} U+${point.toString(16)}. Add a licensed fallback; never omit text.`,
      );
...
function localImage(src, dist) {
  if (!src?.startsWith('/images/') && !src?.startsWith('/_astro/'))
    throw new Error(`PDF requires a generated local image: ${src}`);
  const file = path.resolve(dist, `.${decodeURIComponent(src)}`);
  if (!file.startsWith(`${path.resolve(dist)}${path.sep}`))
    throw new Error('Unsafe PDF image path');
  return file;
}
...
async function typeset(article, dist) {
  ...
  const sourceText = (node) => {
    if (
      ...
      (node.tagName === 'a' && /^#fnref/.test(attr(node, 'href') ?? ''))
    )
      return '';
    return node.nodeName === '#text'
      ? node.value
      : (node.childNodes ?? []).map(sourceText).join('');
  };
  ...
  const extracted = blocks
    ...
    .map((run) => run.text)
    .join('');
  if (normalized(extracted) !== normalized(sourceText(article.prose)))
    throw new Error(`PDF block extraction lost content: ${article.title}`);
  const doc = new PDFDocument({
    size: PAGE,
    margins: { top: 64, bottom: BOTTOM, left: MARGIN, right: MARGIN },
    bufferPages: true,
    autoFirstPage: true,
  });
  ...
  function space(height) {
    if (doc.y + height > PAGE[1] - BOTTOM) doc.addPage();
  }
  function text(
    runs,
    { size = 13.5, style = 'normal', indent = 0, color = '#181613', heading = false } = {},
  ) {
    ...
    for (let i = 0; i < nonempty.length; i++) {
      ...
      doc
        .font(part.font)
        .fontSize(part.sup ? size * 0.72 : size)
        .fillColor(color)
        .text(part.text, {
  ...
  async function image(src, maxHeight) {
    ...
    space(h + 20);
    const y = doc.y;
    doc.image(await sharp(file).png().toBuffer(), MARGIN + (width - w) / 2, y, {
      width: w,
      height: h,
    });
    doc.y = y + h + 18;
  ...
  doc
    .moveTo(MARGIN, doc.y)
    .lineTo(PAGE[0] - MARGIN, doc.y)
    .lineWidth(0.6)
    .stroke('#8B1A1A');
  doc.y += 24;
  text([{ text: article.title }], { size: 28, style: 'bold', heading: true });
  if (article.subtitle)
    text([{ text: article.subtitle }], { size: 15, style: 'italic', color: '#4A4038' });
  if (article.dek) text([{ text: article.dek }], { size: 14, style: 'italic' });
  text(
    [
      {
        text: `${article.author} · ${new Intl.DateTimeFormat('vi-VN', { dateStyle: 'long', timeZone: 'UTC' }).format(new Date(article.date))}`,
      },
    ],
    { size: 10.5, color: '#5B5149' },
  );
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    doc.text(`${i + 1} / ${range.count}`, PAGE[0] - MARGIN - 32, PAGE[1] - 40, {
      lineBreak: false,
    });
  }
  doc.end();
...
async function rasterize({ bytes, expectedText }, title) {
  ...
  const pdf = new PDFDocument({
    ...
    info: {
      Title: title,
      Author: 'The Cochinchine Pensées',
      Subject: 'Bản đọc và in từ ảnh trang',
    },
  });
  ...
  try {
    ...
    for (let i = 1; i <= source.numPages; i++) {
      ...
      extracted += content.items
        .filter((item) => item.str && item.transform[5] > BOTTOM - 5)
        .map((item) => item.str)
        .join('');
      ...
      pdf
        .addPage({ size: PAGE, margin: 0 })
        .image(image, 0, 0, { width: PAGE[0], height: PAGE[1] });
      canvas.width = canvas.height = 1;
      page.cleanup();
    }
    const normalized = (value) => value.normalize('NFKC').replace(/[\s\u200B-\u200D\uFEFF]/g, '');
    if (normalized(extracted) !== normalized(expectedText))
      throw new Error(`PDF text fidelity check failed: ${title}`);
    pdf.end();
...
export async function generateReaderPdfs(dist, { only } = {}) {
  ...
  for (const slug of (await fs.readdir(path.join(dist, 'essays'))).sort()) {
    ...
    const hash = createHash('sha256')
      ...
      .update(
        JSON.stringify({
          ...
          date: article.date,
          body: article.proseHtml,
          cover: attr(article.cover, 'src'),
        }),
      );
    ...
    let entry = JSON.parse(
      await fs.readFile(path.join(cache, `${key}.json`), 'utf8').catch(() => 'null'),
    );
    if (!entry || !(await fs.stat(cached).catch(() => null))) {
      ...
      if (raster.bytes.length > 24 * 1024 * 1024)
        throw new Error(`PDF exceeds static host limit: ${slug}`);
  ...
  await fs.writeFile(
    path.join(root, '.astro/reader-pdf-report.json'),
