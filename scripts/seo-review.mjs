...
export async function validateEditorialSeo({ reportOnly = false } = {}) {
  const profiles = JSON.parse(
    await fs.readFile(path.join(root, 'editorial/essay-seo.json'), 'utf8'),
  );
  const errors = [],
  ...
  for (const file of await fs.readdir(path.join(root, 'src/content/essays'))) {
    ...
    if (!profile || profile.reviewedSourceHash !== hash)
      errors.push(`${id}: review required; reviewedSourceHash = ${hash}`);
    if (!profile) continue;
    ...
    if (
      ...
      errors.push(`${id}: write a concise, plain-text description (60–300 characters).`);
    ...
    for (const field of ['metadataUpdatedAt', 'contentModifiedAt']) {
      ...
      if (
        ...
        errors.push(`${id}: invalid ${field}.`);
    }
    if (!profile.focusQuery?.trim()) errors.push(`${id}: missing internal query hypothesis.`);
  }
  for (const id of Object.keys(profiles))
    if (!published.has(id)) errors.push(`${id}: profile has no published essay.`);
  if (errors.length && !reportOnly)
    throw new Error(`Editorial SEO review failed:\n${errors.join('\n')}\nSee docs/seo.md.`);
