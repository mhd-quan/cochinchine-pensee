...
| Arrow left/right, return to top | Series carousel, article links, return control |
| Arrow up-right | External newsletter destination |
| Chevron left/right | Book carousel and archive pagination, retaining the angle-bracket silhouette |
...
controls use 20px; the mobile Aa uses a bespoke serif capital in a 24px box; arrows alongside labels and the fleuron use 16px. The
...
only geometry and presentation attributes (3,699 bytes total), without editor metadata or
embedded raster images. The CSS asterism uses the same fleuron as a mask.
Menu and copy/check states cross-fade without changing their layout box;
reduced-motion preferences disable those fades.
