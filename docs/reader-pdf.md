...
Facebook dùng `facebook.com/sharer/sharer.php?u=…`; X dùng web intent `twitter.com/intent/tweet` với `text` và `url`; Email dùng `mailto:` với tiêu đề cùng URL chuẩn. Copy ghi URL chuẩn, không mang tracking hoặc fragment. Khi clipboard không có hoặc bị từ chối, hiện ô URL đã chọn để người đọc tự chép. Khởi tạo lại sau chuyển trang Astro và không gắn trùng listener. Từ v0.7.7, Facebook và X là nút thao tác của reader, chỉ mở cửa sổ chia sẻ sau cú bấm với noopener/noreferrer; có liên kết dự phòng khi JavaScript tắt. Không có SDK, tracker hay prefetch của nền tảng. Hàng thao tác nằm ngay sau nguyên văn, trước các module bạn đọc trên mobile.

Từ v0.7.6.1, logo Facebook dùng đúng Secondary Logo trong bộ `Facebook Brand Asset Pack 2/Logo/Secondary Logo` do chủ biên cung cấp. Path vector được trích từ `Facebook_Logo_Secondary.ai` (PDF-compatible Illustrator), tối ưu bằng SVGO ở độ chính xác 3 chữ số thập phân, giữ nguyên hình tròn và phần chữ f trong suốt. Bỏ phần lề artboard khỏi viewBox; logo hiện 20 × 20 px trong nút 44 × 44 px, nên vẫn có 12 px khoảng trống mỗi phía. SVG một path nhúng trực tiếp, không tạo request ảnh hay JavaScript mới. Giữ đen trên Paper/Sepia/Sage và trắng trên Night, kể cả khi hover; không dùng màu crimson cho dấu nền tảng. Logo X lấy nguyên SVG trong [gói logo chính thức](https://about.x.com/content/dam/about-twitter/x/brand-toolkit/x-logo.zip); giữ đen trên nền sáng và trắng trên nền tối theo [X Brand Guidelines](https://about.x.com/content/dam/about-twitter/x/brand-toolkit/x-brand-guidelines.pdf). Copy, Email và PDF là glyph tự vẽ, nét 1.7 theo bộ icon của ấn phẩm.
